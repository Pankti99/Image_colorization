...
def convert_to_bmp():
    ...
    for file in glob.glob("E:/msc AI/Image colorization using scribbles/source/*.png"):
         im = Image.open(file)
         rgb_im = im.convert('RGB')
         rgb_im.save(file.replace("png", "bmp"))
         
    for file in glob.glob("E:/msc AI/Image colorization using scribbles/source/*.jpg"):
         im = Image.open(file)
         rgb_im = im.convert('RGB')
         rgb_im.save(file.replace("jpg","bmp"))
         
    for file in glob.glob("E:/msc AI/Image colorization using scribbles/marked/*.png"):
         im = Image.open(file)
         rgb_im = im.convert('RGB')
         rgb_im.save(file.replace("png", "bmp"))
         
    for file in glob.glob("E:/msc AI/Image colorization using scribbles/marked/*.jpg"):
         im = Image.open(file)
         rgb_im = im.convert('RGB')
         rgb_im.save(file.replace("jpg","bmp"))
...
def read_img(sn,tn):
        ...
        s = cv2.imread('E:/msc AI/Image colorization using scribbles/source/'+str(sn)+".bmp")
        t=cv2.imread('E:/msc AI/Image colorization using scribbles/marked/'+str(tn)+".bmp")
        return s,t

def color():
    count=0
    i=0
#    sources = ['s1','s2','s3','s4','s5','s6','s7','s10','s9','s11']
#    targets = ['t1','t2','t3','t4','t5','t6','t7','t10','t9','t11']
    
    sources = ['s12']
    targets = ['t12']
    convert_to_bmp()
    for i in range(0,len(sources)):
        print("Converting picture"+str(i+1)+"...")
        s, t = read_img(sources[i],targets[i])
        cv2.imshow("Gray Image",s)
        cv2.waitKey(0)
        cv2.imshow("Marked Image",t)
        cv2.waitKey(0)
        x,y,z=s.shape
        x1,y1,z1=t.shape
        if (x!=x1) and (y!=y1)  :
            t = cv2.resize(t, (y,x), interpolation = cv2.INTER_AREA)

       
        original = s.astype(float)/255
     
        marked = t.astype(float)/255
        ...
        result = colorizedRGB.astype(float)*255

        count+=1
        
        cv2.imwrite('E:/msc AI/Image colorization using scribbles/output/r'+str(count)+'.bmp',result)

        cv2.imshow("Colorized Image",colorizedRGB)
        cv2.waitKey(0)
